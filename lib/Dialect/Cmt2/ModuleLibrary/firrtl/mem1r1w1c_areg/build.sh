...
OUTPUT_FILE="Mem1r1w1c_AReg_w${DATA_WIDTH}_a${ADDR_WIDTH}_d${DEPTH}.mlir"
MODULE_NAME="Mem1r1w1c_AReg_w${DATA_WIDTH}_a${ADDR_WIDTH}_d${DEPTH}"
...
module {
  firrtl.circuit "${MODULE_NAME}" {
    firrtl.module @${MODULE_NAME}(
      ...
      // Register to hold the read address when ren is low
      %raddr_reg = firrtl.reg %clock : !firrtl.clock, !firrtl.uint<${ADDR_WIDTH}>
      // Wire for actual read address (mux output)
      %actual_raddr = firrtl.wire : !firrtl.uint<${ADDR_WIDTH}>

      firrtl.when %ren : !firrtl.uint<1> {
        firrtl.matchingconnect %raddr_reg, %raddr : !firrtl.uint<${ADDR_WIDTH}>
        firrtl.matchingconnect %actual_raddr, %raddr : !firrtl.uint<${ADDR_WIDTH}>
      } else {
        firrtl.matchingconnect %actual_raddr, %raddr_reg : !firrtl.uint<${ADDR_WIDTH}>
      }
      ...
      firrtl.matchingconnect %r_addr, %actual_raddr : !firrtl.uint<${ADDR_WIDTH}>
      
      %c1 = firrtl.constant 1 : !firrtl.uint<1>
      firrtl.matchingconnect %r_en, %c1 : !firrtl.uint<1>
