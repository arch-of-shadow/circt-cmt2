...
class _TaskDef:
    ...
    def __call__(self, func: F) -> F:
        task_name = self.alias or _get_function_name(func)
...
class DataflowContext:
    ...
    def task(
        ...
        *,
        ...
        if func is not None:
            return _TaskDef(self, alias=alias, tokens_in=tokens_in, tokens_out=tokens_out, timing=timing)(func)
        return _TaskDef(self, alias=alias, tokens_in=tokens_in, tokens_out=tokens_out, timing=timing)
...
class _DataflowDef:
    def __init__(self, module_builder: Any, alias: str | None = None, interval: int | None = None):
        self._builder = _unwrap_module_builder(module_builder)
        self._alias = alias
        self._interval = interval

    def __call__(self, func: F) -> F:
        df_name = self._alias or _get_function_name(func)
...
def dataflow(module_builder: Any, *, alias: str | None = None, interval: int | None = None) -> _DataflowDef:
    """Define a dataflow pipeline (decorator form)."""
    return _DataflowDef(module_builder, alias=alias, interval=interval)
