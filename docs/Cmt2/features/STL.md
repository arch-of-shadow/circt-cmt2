...
## Simulation & “Where the RTL Comes From”

STL instances can be backed by either:

- **Cmt2-defined modules** (rules/methods/values lowered like any other Cmt2 module), or
- **ModuleLibrary-backed extern modules** (FIRRTL `extmodule` / Cmt2 extern bindings)

In generated SystemVerilog, extern-backed STL blocks may appear as references to
module definitions that are provided by the ModuleLibrary.

### Workspace self-contained vs “single SV file” self-contained

The emitted design SV may reference external module definitions:

```
Top.sv  --->  Reg_width32_init0 (extern)
        --->  Mem_*             (extern)
```

`SimulationWorkspace` makes the **simulation workspace** self-contained by:

- Detecting which ModuleLibrary module *variants* are referenced (exact widths/params)
- Emitting only the missing RTL modules into `rtl/` (no duplicates if already present)

This is why you may see “prebuilt reg/FSM” modules in the sim workspace: they are
real hardware state elements introduced by lowering, implemented using common
ModuleLibrary primitives.

For GAA-level stepping, `cmt2-dbg` interprets Cmt2 semantics; for cycle-accurate
behavior (especially for library FIFO/memory variants), prefer RTL simulation.

---

## Implementation Notes (How STL Is Represented)

At the IR level, STL components are typically represented as:

- A **FIRRTL extern module** binding (`cmt2.module.extern.firrtl`) describing the
  port-level interface and scheduling constraints, and
- An **instance** of that extern module inside a Cmt2 module.

Manual “extract” commands:

- Find extern-module usage in examples/tests:
  - `rg -n \"cmt2\\.module\\.extern\\.firrtl\" test/Dialect/Cmt2 examples | head`
- Find the Python STL builders:
  - `rg -n \"pycmt2\\.stl\" -S lib/Bindings/Python tools | head`

These bindings ensure the compiler knows:
- method/value names and ports,
- ready/enable/data conventions, and
- scheduling relationships (conflict/ordering) for correct rule scheduling.
...
- [Debugging.md](../guides/Debugging.md) - Simulation and debugging
