...
Usage:
    ...
    PYTHONPATH=tools/circt/python_packages/circt_core python3 \
        ../examples/PyCMT2/li_token_pipeline.py
"""
...
def create_packet_processor():
    ...
    with circuit.module("PacketPipeline") as mod:
        clk = mod.clock()
        rst = mod.reset()
        ...
        # =====================================================================
        with mod.method("increment_count") as meth:
        ...
        with mod.value("get_packet_count", returns=[UInt(16)]) as val:
            with val.guard() as g:
                g.always()
            with val.body() as b:
                count = b.call(packet_count, "read")
        ...
        with mod.value("get_status", returns=[UInt(8)]) as val:
            with val.guard() as g:
                g.always()
            with val.body() as b:
